...
class DatasetsStorageClient:
    ...
    def _initialize_table(self):
        ...
        CREATE INDEX IF NOT EXISTS idx_name ON {table} (filename);
        """).format(table=sql.Identifier(self.table_name))
        
        with self._get_cursor() as cursor:
            cursor.execute(create_table_query)

    def create_object(self, filename: str, data: Dict[str, Any]) -> Dict[str, Any]:
        query = sql.SQL("""
        INSERT INTO {table} (filename, data)
        VALUES (%s, %s)
        RETURNING id, filename, data, created_at, updated_at;
        """).format(table=sql.Identifier(self.table_name))
        
        with self._get_cursor() as cursor:
            cursor.execute(query, (filename, json.dumps(data)))
    ...
    def list_objects(self, limit: int = 100) -> List[Dict[str, Any]]:
        query = sql.SQL("""
        SELECT id, filename, data, created_at, updated_at
        FROM {table}
